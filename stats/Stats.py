...
class Stats():
	...
	def runSimpleSimulation(self, decision):
		...
		for hid, cids in enumerate(host_alloc):
			ips = 0
			for cid in cids: ips += self.env.containerlist[cid].getApparentIPS()
			e = self.env.hostlist[hid].getPowerFromIPS(ips)
			emax = self.env.hostlist[hid].getPowerFromIPS(self.env.hostlist[hid].ipsCap)
			energytotalinterval_pred += (e / emax)
		return energytotalinterval_pred / len(host_alloc)
